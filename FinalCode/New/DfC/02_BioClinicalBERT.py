...
# BioClinicalBERT Fine-Tuning Wrapper (for Unstructured Text)
...
def apply_bioclinicalbert_on_patient_notes(df, note_columns, tokenizer, model, device, aggregation="mean", max_length=128):
    """
    For each unique patient in the DataFrame, the function extracts all valid clinical notes
    from the specified note_columns, applies tokenization and passes them through the BioClinicalBERT model.
    The embeddings are then aggregated (by mean or max) to yield one embedding per patient.
    """
...
# Unstructured Dataset and Classifier Definitions
...
class UnstructuredClassifier(nn.Module):
    def __init__(self, input_size=768, hidden_size=256):
        ...
        # This classifier outputs 3 logits corresponding to mortality, LOS, and mechanical ventilation.
...
# Training and Evaluation Functions for Unstructured Model
...
# Fairness Evaluation (EDDI) Utilities
...
def train_pipeline():
    ...
    # Identify note columns (assume they start with 'note_').
    note_columns = [col for col in df.columns if col.startswith("note_")]
    print("Found note columns:", note_columns)
    
    # Filter rows: keep only rows that have at least one valid note.
    ...
    # Prepare the tokenizer and BioClinicalBERT model.
    ...
    print("Computing aggregated text embeddings...")
    ...
    # For DfC, the model only uses unstructured text embeddings.
    # Outcome labels and sensitive attributes for evaluation are kept separately.
    ...
    # Instantiate the classifier. (Since we're using only text, this is our DfC model.)
    classifier = UnstructuredClassifier(input_size=768, hidden_size=256).to(device)
    
    # Compute class weights for each outcome (based on df_unique).
    ...
    # Inference: get patient-level probabilities.
    probs = get_patient_probabilities(classifier, dataloader, device)
    
    # For fairness evaluation, merge predictions with demographics from df_unique.
    ...
    # Standardize sensitive attribute names (for fairness evaluation).
    ...
    df_results.to_csv("dfc_bioclinicalbert_predictions_and_fairness.csv", index=False)
    print("Results saved to 'dfc_bioclinicalbert_predictions_and_fairness.csv'.")
