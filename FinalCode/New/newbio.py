...
class UnstructuredDataset(Dataset):
    def __init__(self, embeddings, mortality_labels, los_labels):
        """
        embeddings: NumPy array of shape (num_patients, hidden_size)
        mortality_labels, los_labels: arrays/lists of binary labels
        """
    ...
    def __getitem__(self, idx):
        return self.embeddings[idx], self.mortality_labels[idx], self.los_labels[idx]
...
# Training Function for Unstructured Classifier
def train_model(model, dataloader, optimizer, device, criterion_mort, criterion_los):
    model.train()
    running_loss = 0.0
    for batch in dataloader:
        embeddings, labels_mort, labels_los = [x.to(device) for x in batch]
        optimizer.zero_grad()
...
def evaluate_model(model, dataloader, device, threshold=0.5):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            all_labels.append(torch.cat((labels_mort.cpu(), labels_los.cpu()), dim=1))
    all_logits = torch.cat(all_logits, dim=0).numpy()
    all_labels = torch.cat(all_labels, dim=0).numpy()
    probs = expit(all_logits)  # Sigmoid probabilities
...
def train_pipeline():
    ...
    df = pd.read_csv("filtered_unstructured_dataset.csv", low_memory=False)
    ...
    # Use mortality and LOS outcomes.
    mortality_labels = df_unique["short_term_mortality"].values
    los_labels = df_unique["los_gt_3"].values

    # Build dataset and dataloader.
    dataset = UnstructuredDataset(aggregated_embeddings_np, mortality_labels, los_labels)
    ...
    class_weights_los = compute_class_weights(df_unique, "los_gt_3")
    ...
    for epoch in range(num_epochs):
        train_loss = train_model(classifier, dataloader, optimizer, device, criterion_mort, criterion_los)
    ...
    # Build a results DataFrame merging predictions with demographics.
    ...
    # Merge true labels from df_unique.
    df_results = pd.merge(df_results,
                          df_unique[['subject_id', 'short_term_mortality', 'los_gt_3']],
    ...
    # Fairness Evaluation: LOS Outcome.
    ...
    print("Overall EDDI:", overall_eddi_los)
