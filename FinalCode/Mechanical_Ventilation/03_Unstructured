...
# Utility Functions for Demographics and Text Preprocessing
...
structured_file = 'structured_dataset.csv'  # This file must contain the 'mechanical_ventilation' flag

# Read Admissions, ICU Stays, Patients, and Notes Data
...
# Notes (CHARTDATE may only be a date)
...
# Merge Admissions, ICU Stays, and Patients for Structured Data
...
# Calculate Patient Age and Create Age Categories
...
# Save the structured dataset (this will later be merged with the unstructured notes)
...
# UNSTRUCTURED NOTES: Process Notes During the ICU Stay
...
# Merge these notes with admission and discharge times and the mechanical ventilation flag.
first_icu_admission = df_first_icu[['subject_id', 'hadm_id', 'INTIME', 'DISCHTIME', 'mechanical_ventilation']].copy()
...
# Compute hours since admission (using CHARTDATE as the note timestamp)
...
# Aggregate notes by subject and hadm_id by concatenating all TEXT entries and retaining the mechanical ventilation flag.
notes_agg = notes_filtered.groupby(['subject_id', 'hadm_id']).agg({
    'TEXT': lambda texts: " ".join(texts),
    'mechanical_ventilation': 'first'
}).reset_index()
...
# Merge Demographics and Outcome into Unstructured Dataset Using Common subject_id
...
# Load the structured dataset that contains mechanical ventilation
structured_df = pd.read_csv(structured_file)
print(f"Structured data shape: {structured_df.shape}")

# Ensure both datasets have common subject_ids by filtering them:
...
unstructured_df = unstructured_df[unstructured_df['subject_id'].isin(common_ids)].copy()
structured_df = structured_df[structured_df['subject_id'].isin(common_ids)].copy()

# Optionally, create an age bucket in structured_df if not present
...
# Select key demographic columns along with mechanical ventilation
demo_cols = ['subject_id', 'age', 'age_bucket', 'ethnicity_category', 'insurance_category', 'gender', 'mechanical_ventilation']
...
# Merge demographics and mechanical ventilation outcome into the unstructured dataset.
merged_unstructured = pd.merge(unstructured_df, structured_df[demo_cols], on='subject_id', how='left')
print("Merged unstructured dataset shape (with demographics and mechanical ventilation):", merged_unstructured.shape)

# Save the final unstructured dataset with demographics and the mechanical ventilation outcome.
...
ventilated_count = merged_unstructured['mechanical_ventilation'].sum()
print("Number of ICU stays with mechanical ventilation:", ventilated_count)
