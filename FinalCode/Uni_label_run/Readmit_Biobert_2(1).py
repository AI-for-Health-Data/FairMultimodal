...
# Dataset for Readmission Only
class UnstructuredDataset(Dataset):
    def __init__(self, embeddings, readmission_labels):
        """
        embeddings: a NumPy array of shape (num_patients, hidden_size)
        readmission_labels: 1-D array/list of labels (0 or 1)
        """
    ...
    def __getitem__(self, idx):
        return self.embeddings[idx], self.readmission_labels[idx]

# Classifier for Readmission Only
class UnstructuredClassifier(nn.Module):
    def __init__(self, input_size=768, hidden_size=256):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(hidden_size, 1)  # Single output: readmission
        )
...
# Training function for Readmission Only
def train_model(model, dataloader, optimizer, device, criterion_readm):
    model.train()
    running_loss = 0.0
    for batch in dataloader:
        embeddings, labels_readm = [x.to(device) for x in batch]
        optimizer.zero_grad()
        logits = model(embeddings)  # shape: (batch_size, 1)
        loss_readm = criterion_readm(logits, labels_readm)
        loss = loss_readm  # Only readmission loss is used
        loss.backward()
        optimizer.step()
        running_loss += loss.item()
    return running_loss / len(dataloader)

# Evaluation function for Readmission Only
def evaluate_model(model, dataloader, device, threshold=0.5):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            all_labels.append(labels_readm.cpu())
    ...
    metrics["readmission"] = {"aucroc": aucroc, "auprc": auprc, "f1": f1, "recall": recall, "precision": precision}
    return metrics
...
def train_pipeline():
    ...
    # Get readmission labels from unique patients
    readmission_labels = df_filtered.drop_duplicates(subset="subject_id")["readmission_within_30_days"].values

    dataset = UnstructuredDataset(aggregated_embeddings_np, readmission_labels)
    ...
    for epoch in range(num_epochs):
        train_loss = train_model(classifier, dataloader, optimizer, device, criterion_readm)
