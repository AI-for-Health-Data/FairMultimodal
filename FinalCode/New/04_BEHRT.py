...
# IMPORTANT: Fill missing age values to avoid NaNs.
...
def prepare_sequences(df):
    ...
    for patient in patients:
        ...
        # Use ward columns if available; otherwise use placeholder 0.
        ...
        # Labels: short-term mortality and LOS (>3 days).
        mortality_label = patient_data['short_term_mortality'].max()
        los_label = patient_data['los_gt_3'].max()
        ...
        labels.append([mortality_label, los_label])
...
class BEHRTModel(nn.Module):
    def __init__(self, num_diseases, num_ages, num_segments, num_admission_locs, num_discharge_locs,
        ...
        # Two classifiers: one for mortality and one for LOS.
    ...
    def forward(self, input_ids, age_ids, segment_ids, admission_loc_ids, discharge_loc_ids,
        ...
        return logits_mortality, logits_los

# Define hyperparameters based on dataset statistics.
num_diseases = len(disease_mapping)
num_ages = int(df_filtered['age'].nunique() + 1)  # add 1 to include zero
num_segments = 2
...
# Compute class weights for mortality and LOS.
...
for epoch in range(epochs):
    ...
    for batch in dataloader:
        ...
        optimizer.zero_grad()
        logits_mortality, logits_los = model(
...
def evaluate_model(model, dataloader, device):
    model.eval()
    all_labels = []
    all_logits = {'mortality': [], 'los': []}
    all_predictions = {'mortality': [], 'los': []}
    with torch.no_grad():
        for batch in dataloader:
            input_ids_b, age_ids_b, segment_ids_b, admission_loc_ids_b, discharge_loc_ids_b, \
            gender_ids_b, ethnicity_ids_b, insurance_ids_b, labels_b = batch
    ...
    return {
        ...
        'labels': all_labels,
        'auroc': {'mortality': auroc_mortality, 'los': auroc_los},
        'auprc': {'mortality': auprc_mortality, 'los': auprc_los},
        'precision': {'mortality': precision_mortality_score, 'los': precision_los_score},
        'recall': {'mortality': recall_mortality_score, 'los': recall_los_score},
        'f1': {'mortality': f1_mortality, 'los': f1_los}
    }
...
def get_model_predictions(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            all_predictions['los'].extend(pred_los)
            all_labels.append(labels_b.cpu().numpy())
    all_labels = np.concatenate(all_labels, axis=0)
    return all_predictions, all_labels

predictions, labels_arr = get_model_predictions(model, dataloader, device)
predictions['mortality'] = np.array(predictions['mortality'])
predictions['los'] = np.array(predictions['los'])
...
fairness_results_gender = evaluate_fairness(labels_arr[:, 0], predictions['mortality'], sensitive_attribute_gender)
...
def get_model_predictions(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Passing an attention mask (assume 0 is the padding token).
            logits_mortality, logits_los = model(
                input_ids=input_ids,
                age_ids=age_ids,
            ...
            # For demonstration, suppose we're using mortality predictions.
...
# Create a DataFrame copy to extract sensitive attributes.
df_filtered_copy = df_filtered.copy()

# Update age bins to cover all ages (including 0) so that no records fall out of range.
age_bins = [15, 30, 50, 70, 90]
age_labels_bins = ['15-29', '30-49', '50-69', '70-89']
df_filtered_copy['age_group'] = pd.cut(df_filtered_copy['age'], bins=age_bins, labels=age_labels_bins, right=False)

# Convert the age group values to string (this ensures consistent keys).
df_filtered_copy['age_group'] = df_filtered_copy['age_group'].astype(str)

# For ethnicity, use the existing categorized column.
df_filtered_copy['ethnicity_group'] = df_filtered_copy['categorized_ethnicity']

# For insurance, create a grouping based on the INSURANCE code.
df_filtered_copy['insurance_group'] = df_filtered_copy['INSURANCE'].replace({
    0: 'Government', 1: 'Medicaid', 2: 'Medicare', 3: 'Private', 4: 'Self Pay'
})

# Get sensitive attribute arrays.
age_groups = df_filtered_copy['age_group'].values
ethnicity_groups = df_filtered_copy['ethnicity_group'].values
insurance_groups = df_filtered_copy['insurance_group'].values

def compute_eddi(sensitive_attr, true_labels, pred_labels):
    """
    Computes subgroup error disparities.
    Returns a dictionary mapping each subgroup to its disparity 
    (relative error difference from the overall error rate) and the overall error rate.
    """
...
# Print detailed EDDI for each individual age group.
...
# Combine attribute-level EDDI scores (for demonstration, using the root mean square divided by 3).
...
outcome_names = {'mortality': 'Short-term Mortality', 'los': 'LOS > 3 Days'}
outcome_indices = {'mortality': 0, 'los': 1}
